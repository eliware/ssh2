...
function parseDER(data, baseType, comment, fullType) {
  // OpenSSH host/user certificates wrap the underlying public key fields
  // with a nonce before the normal key payload.  The certificate signature
  // is verified with that underlying key, so expose it using the same parsed
  // key implementation while preserving the certificate type for callers.
  if (baseType.endsWith('-cert-v01@openssh.com')) {
    const certBaseType = baseType.slice(0, -'-cert-v01@openssh.com'.length);
    if (!skipFields(data, 1))
      return new Error('Malformed OpenSSH certificate');
    baseType = certBaseType;
  }
